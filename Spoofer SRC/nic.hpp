...
#define NSI_PARAMS_ARP (11)
	...
	bool spoofer_nic()
	{
		...
		DWORD64 address = 0;
		DWORD32 size = 0;
		if (n_util::get_module_base_address(XorString("ndis.sys"), address, size) == false) return false;
		n_log::printf(XorString("ndis address : %llx \t size : %x \n"), address, size);

		PNDIS_FILTER_BLOCK ndis_global_filter_list = (PNDIS_FILTER_BLOCK)n_util::find_pattern_image(address,
			XorString("\x40\x8A\xF0\x48\x8B\x05"),
			XorString("xxxxxx"));
		if (ndis_global_filter_list == 0) return false;
		n_log::printf(XorString("ndis global filter list address : %llx \n"), ndis_global_filter_list);

		DWORD64 ndis_filter_block = n_util::find_pattern_image(address,
			XorString("\x48\x85\x00\x0F\x84\x00\x00\x00\x00\x00\x8B\x00\x00\x00\x00\x00\x33"),
			XorString("xx?xx?????x???xxx"));
		if (ndis_filter_block == 0) return false;
		n_log::printf(XorString("ndis filter block address : %llx \n"), ndis_filter_block);

		DWORD ndis_filter_block_offset = *(PDWORD)((PBYTE)ndis_filter_block + 12);
		if (ndis_filter_block_offset == 0) return false;
		n_log::printf(XorString("ndis filter block offset value : %x \n"), ndis_filter_block_offset);
