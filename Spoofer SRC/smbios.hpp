...
namespace n_smbios
{
	char smbios_vendor[100]{ 0 };
	char smbios_version[100]{ 0 };
	char smbios_date[100]{ 0 };
	char smbios_manufacturer[100]{ 0 };
	char smbios_product_name[100]{ 0 };
	char smbios_serial_number[100]{ 0 };
	...
	void process_smbios_table(SMBIOS_HEADER* header)
	{
		...
		if (header->Type == 0)
		{
			...
			if (type_0)
			{
				...
				if (vendor)
				{
					n_log::printf(XorString("old vendor : %s \n"), vendor);
					RtlCopyMemory(vendor, smbios_vendor, strlen(vendor));
					n_log::printf(XorString("new vendor : %s \n"), n_util::random_string(vendor, 0));
				}

				if (version)
				{
					n_log::printf(XorString("old version : %s \n"), version);
					RtlCopyMemory(version, smbios_version, strlen(version));
					n_log::printf(XorString("new version : %s \n"), n_util::random_string(version, 0));
				}

				if (date)
				{
					n_log::printf(XorString("old date : %s \n"), date);
					RtlCopyMemory(date, smbios_date, strlen(date));
					n_log::printf(XorString("new date : %s \n"), n_util::random_string(date, 0));
				}
			}
		}
		...
		else if (header->Type == 1)
		{
			...
			if (type_1)
			{
				...
				if (manu_facturer)
				{
					n_log::printf(XorString("old manufacturer : %s \n"), manu_facturer);
					RtlCopyMemory(manu_facturer, smbios_manufacturer, strlen(manu_facturer));
					n_log::printf(XorString("new manufacturer : %s \n"), n_util::random_string(manu_facturer, 0));
				}

				if (product_name)
				{
					n_log::printf(XorString("old product name : %s \n"), product_name);
					RtlCopyMemory(product_name, smbios_product_name, strlen(product_name));
					n_log::printf(XorString("new product name : %s \n"), n_util::random_string(product_name, 0));
				}

				if (serial_number)
				{
					n_log::printf(XorString("old serial number : %s \n"), serial_number);
					RtlCopyMemory(serial_number, smbios_serial_number, strlen(serial_number));
					n_log::printf(XorString("new serial number : %s \n"), n_util::random_string(serial_number, 0));
				}
	...
	bool spoofer_smbios()
	{
		DWORD64 address = 0;
		DWORD32 size = 0;
		if (n_util::get_module_base_address(XorString("ntoskrnl.exe"), address, size) == false) return false;
		n_log::printf(XorString("ntoskrnl address : %llx \t size : %x \n"), address, size);
		...
		PPHYSICAL_ADDRESS physical_address = (PPHYSICAL_ADDRESS)n_util::find_pattern_image(address,
			XorString("\x48\x8B\x0D\x00\x00\x00\x00\x48\x85\xC9\x74\x00\x8B\x15"),
			XorString("xxx????xxxx?xx"));
		...
		if (physical_address == 0) return false;
		n_log::printf(XorString("physical address : %llx \n"), physical_address);

		// WmipFindSMBiosStructure -> WmipSMBiosTableLength
		DWORD64 physical_length_address = n_util::find_pattern_image(address,
			XorString("\x8B\x1D\x00\x00\x00\x00\x48\x8B\xD0\x44\x8B\xC3\x48\x8B\xCD\xE8\x00\x00\x00\x00\x8B\xD3\x48\x8B"),
			XorString("xx????xxxxxxxxxx????xxxx"));
		...
		n_log::printf(XorString("physical length : %d \n"), physical_length);
